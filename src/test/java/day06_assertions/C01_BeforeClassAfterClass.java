...
public class C01_BeforeClassAfterClass {
    ...
    public void test02() {      // 2.method'da amazon'da nutella aratip, sonuclarin nutella icerdigini test edin
        ...
        if (searchResultSTR.contains(expectedWord)){
            System.out.println("Nutella seacrh test passed");
        }else {
    ...
    @Test
    public void test03() {      // 3.method'da nutella arama sonuc sayisinin 50'den fazla oldugunu test edin
        WebElement searchResult = driver.findElement(By.xpath("//h1[@class='a-size-base s-desktop-toolbar a-text-normal']"));
        String searchResultSTR = searchResult.getText();
    ...
    //  System.out.println(searchResultSTR);  // "48 results for 'Nutella'" cikti. Burdan 48 sayisini integer olarak almaliyiz:
        ...
        String resultCount = searchResultArr[0]; // 48
        int resultCountInt = Integer.parseInt(resultCount);

        int expectedResultCount = 50;

        if (resultCountInt>expectedResultCount) {
            System.out.println("Nutella count search test Passed");
        }else {
            System.out.println("Nutella count search test Failed");
